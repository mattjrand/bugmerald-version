...
SINGLE_BATTLE_TEST("Shield Dust blocks powder and spore moves")
{
    GIVEN {
        ASSUME(gMovesInfo[MOVE_STUN_SPORE].powderMove);
        PLAYER(SPECIES_WYNAUT);
        OPPONENT(SPECIES_PINECO) { Ability(ABILITY_SHIELD_DUST); }
    } WHEN {
        TURN { MOVE(player, MOVE_STUN_SPORE); }
    } SCENE {
        ABILITY_POPUP(opponent, ABILITY_SHIELD_DUST);
...
SINGLE_BATTLE_TEST("Shield Dust prevents ability stat changes")
{
    GIVEN {
...
SINGLE_BATTLE_TEST("Overcoat prevents ability stat changes")
{
    GIVEN {
